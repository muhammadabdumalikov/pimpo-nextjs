...
export default function SellersReport() {
  ...
  const kpis = (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <ReportKpi label={t("reportsPage.seller")} value={formatNumber(totals.sellers)} />
      <ReportKpi label={t("reportsPage.orders")} value={formatNumber(totals.orders)} />
      <ReportKpi label={t("reportsPage.revenue")} value={money(totals.revenue)} tone="success" />
      <ReportKpi label={t("reportsPage.avgCheck")} value={money(totals.avgCheck)} />
    </div>
  );
