...
export default function CustomersReport() {
  ...
  const kpis = data ? (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <ReportKpi label={t("reportsPage.newCustomers")} value={formatNumber(data.totals.newCustomers)} tone="success" />
      <ReportKpi label={t("reportsPage.returningCustomers")} value={formatNumber(data.totals.returningCustomers)} />
      <ReportKpi label={t("reportsPage.revenue")} value={money(data.totals.revenue)} />
      <ReportKpi label={t("reportsPage.avgCheck")} value={money(data.totals.avgCheck)} />
    </div>
  ) : null;
